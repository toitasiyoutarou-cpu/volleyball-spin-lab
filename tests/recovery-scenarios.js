...
await demo();$('autoRecovery').checked=false;$('end').value='143';
...
await demo();$('autoRecovery').checked=false;$('end').value='143';let blank=true;
...
await demo();$('autoRecovery').checked=false;$('end').value='95';trackFrame=(p,a)=>{const tr=originalTrack(p,a);if(state.frame===20){tr.edgeScore=0;tr.edgeCoverage=0}return tr};await analyze();
...
await demo();$('autoRecovery').checked=false;$('end').value='144';$('stride').value='2';$('trackingMode').value='interpolate';$('trackingMode').onchange();state.anchors=state.anchors.filter(a=>a.frame===0);const noRunCalls=calls;
...
await demo();$('autoRecovery').checked=false;$('stride').value='1';$('end').value='95';state.roi={x:250,y:184,r:15};saveAnchor();await analyze();assert.equal(state.failure.kind,'small');assert($('recoveryNext').textContent.includes('不自然に広げても'));assert.equal(state.rows.length,0);assert.equal($('trimEnd').disabled,true);
...
await demo();$('autoRecovery').checked=false;$('end').value='95';trackFrame=(p,a)=>{const tr=originalTrack(p,a);if(state.frame===20){tr.edgeScore=0;tr.edgeCoverage=0}return tr};await analyze();await $('trimEnd').click();assert.equal(Number($('end').value),19);assert($('status').textContent.includes('最低48枚'));assert.equal(state.run,null);
...
await demo();$('autoRecovery').checked=false;$('end').value='95';trackFrame=originalTrack;readFrame=async i=>{if(i===20)throw Error('test decode failure');return originalRead(i)};await analyze();assert.equal(state.failure.kind,'decode');assert.equal(state.failure.frame,20);assert.equal(state.frameReady,false);assert.equal($('saveResume').disabled,true);const anchorsBefore=state.anchors.length;assert.equal(saveAnchor(),false);assert.equal(state.anchors.length,anchorsBefore);
